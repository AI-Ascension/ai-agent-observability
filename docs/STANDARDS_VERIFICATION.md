...
| bash tests/compose-policy-regressions.sh | Passed 1 positive and 33 negative structured-model fixtures |
| bash tests/compose-source-regressions.sh | Passed 1 positive and 9 negative rendered-source fixtures |
...
dependency, network, port and volume topology. It rejects arbitrary services,
host credential paths, driver bind options, host namespaces, devices, unreviewed
capabilities, and unconfined security options. It requires loopback host publishes,
the six named data volumes, three read-only configuration binds, the external
network, and Laminar/provider telemetry opt-outs. Negative fixtures remove or
weaken each covered property. Existing text tripwires remain supplemental; neither
them nor the structured filter is a complete deployment security analysis.
