...
- Query helper correction: one repeatable-read, locked PostgreSQL snapshot emits the operator restore SQL and ID/digest together; replacement preassigns its row ID, compares the exact snapshot under a project-first lock, and rollback keeps an unknown outcome unknown while reconciling that candidate ID and expected fields, compensating only an exact unchanged row and retaining the candidate key/evidence if reconciliation fails
